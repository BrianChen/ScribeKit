...
interface GenerateResult {
  ok: boolean;                  // false when confidence is LOW | NONE — check before consuming payload
...
`ok` mirrors `response.ok` from the fetch API. When `ok: false`, the place was not confirmed — `editorialContent` and `researchNotes` are empty, `confidence` tells how close identification got, and `errors` carries the human-readable reason. `latitude: 0, longitude: 0` when `ok: false` — always check `ok` before using location data.

## Errors

`generate()` throws `ScribeKitError` for infrastructure failures (rate limit, auth, network error, agent crash). Import it alongside `generate`:
...
try {
  ...
  if (!result.ok) { /* place not found */ }
} catch (e) {
  if (e instanceof ScribeKitError) {
    console.log(e.cause); // original provider or LangGraph error
  }
}
```

`e.cause` holds the original error with all provider fields intact (`status`, `type`, `requestID`, `headers`, etc. for Anthropic). See JSDoc on `ScribeKitError` in `src/errors.ts` for the full shape.
